...
function AddPortfolio() {
  const { register, handleSubmit, formState: { errors, isSubmitting }, watch, reset, setValue, trigger } = useForm({
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      reset(); 
        ...
        const fieldError = error.response?.data?.field; // Check if backend sends specific field error
        ...
        // Potentially focus the field if backend indicates which one failed (e.g., slug unique error)
        // if (fieldError) {
        //   // You might need a way to map fieldError string to the actual input ref
        //   // Example: if (fieldError === 'slug') { slugInputRef.current?.focus(); }
        // }
    }
  };
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 sm:p-8 lg:p-10 w-full max-w-3xl border border-blue-100">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="border border-gray-200 rounded-lg p-4 space-y-4 bg-gray-50/50">
                ...
                <fieldset disabled={isSubmitting}> {/* Disable radio buttons during submission */}
