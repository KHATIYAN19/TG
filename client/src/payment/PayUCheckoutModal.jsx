...
export default function PayUCheckoutModal({
  ...
  useEffect(() => {
    ...
    const handleEscape = (event) => {
      if (event.key === "Escape" && !paymentLoading) {
        onClose();
      }
    };
  ...
  const submitToPayU = (paymentUrl, paymentData) => {
  ...
  const handlePayment = async (event) => {
    ...
    try {
      ...
      // Browser now leaves Target Trek and POSTs directly to PayU.
      submitToPayU(paymentUrl, paymentData);
    } catch (error) {
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              <p className="line-clamp-2 text-sm font-bold text-slate-900">
                {product?.title || "Target Trek Ebook"}
              </p>
